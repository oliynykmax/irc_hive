...
Bot::Bot(const Config &c)
    : sockfd(-1), cfg(c), currentNick(cfg.nick), baseNick(cfg.nick),
      autoJoined(false), nick_attempt(0), backoff(2), backoff_max(30),
      retries(0) {
...
void Bot::send_privmsg(const std::string &target, const std::string &text) {
  send_line("PRIVMSG " + target + " :" + text);
}

void Bot::join_channels() {
...
std::string
Bot::extract_nick_from_prefix(const std::optional<std::string> &prefix) {
...
void Bot::setup_builtin_commands() {
  ...
  add_command("!ping",
              [&, reply](const std::string &target, const std::string &args) {
                (void)args;
                reply(target, "pong!");
              });

  add_command("!help", [&, reply](const std::string &target,
                                  const std::string &args) {
    (void)args;
    reply(target, "/part {message} - leave the channel, with a message");
    reply(target, "/kick [nick] {message} - kick the user out of the channel, "
                  "with a message");
    reply(target,
          "/quit {message} - disconnect from the server, with a message");
    reply(target, "/mode [l/o/t/k/i] - sets the channel modes, i-invite mode, "
                  "o-operator, t-topic, l-limit, k-key");
  ...
  add_command("!filter", [&, reply](const std::string &target,
                                    const std::string &args) {
    if (target.empty() || target[0] != '#') {
      std::string reply_target =
          (target == currentNick && !last_sender_nick.empty())
              ? last_sender_nick
              : target;
    ...
    if (chanIt == channel_ops.end() ||
        chanIt->second.find(last_sender_nick) == chanIt->second.end()) {
      reply(target, "You are not a channel operator - cannot modify filter");
      return;
    }
    if (args.empty()) {
      reply(target, "Usage: !filter word [word...]");
      return;
    }
    ...
    while (iss >> w) {
      ...
      if (!lw.empty())
        if (filter.insert(lw).second)
          ++added;
    }
    reply(target, "Filter updated: " + std::to_string(filter.size()) +
                      " words total (" + std::to_string(added) + " new)");
  });

  add_command("!unfilter", [&, reply](const std::string &target,
                                      const std::string &args) {
    if (target.empty() || target[0] != '#') {
      std::string reply_target =
          (target == currentNick && !last_sender_nick.empty())
    ...
    if (args.empty()) {
      reply(target, "Usage: !unfilter word [word...]");
      return;
    }
    auto fit = channel_filters.find(target);
    if (fit == channel_filters.end() || fit->second.empty()) {
      reply(target, "Filter empty");
      return;
    }
    std::istringstream iss(args);
    std::string w;
    int removed = 0;
    auto &filter = fit->second;
    while (iss >> w) {
      std::string lw;
      lw.reserve(w.size());
      for (char c : w)
        lw.push_back(
            static_cast<char>(std::tolower(static_cast<unsigned char>(c))));
      if (!lw.empty())
        removed += filter.erase(lw);
    }
    reply(target, "Filter updated: " + std::to_string(filter.size()) +
                      " words total (" + std::to_string(removed) + " removed)");
  });

  add_command("!listfilter", [&, reply](const std::string &target,
                                        const std::string &args) {
    (void)args;
    if (target.empty() || target[0] != '#') {
      std::string reply_target =
    ...
    reply(target,
          "Filter(" + std::to_string(fit->second.size()) + "): " + list);
  });
}
...
void Bot::handle_message(const Message &msg) {
  ...
  if (msg.command == "PING") {
    ...
    send_line("PONG :" + token);
    return;
  }

  // Welcome / end of MOTD
  if (msg.command == "001" || msg.command == "376" || msg.command == "422") {
    if (!autoJoined && !cfg.channels.empty()) {
      join_channels();
      autoJoined = true;
    }
    return;
  ...
  if (msg.command == "433") { // nick in use
    ++nick_attempt;
    currentNick = baseNick + "_" + std::to_string(nick_attempt);
  ...
  if (msg.command == "353" && msg.params.size() >= 4) { // RPL_NAMREPLY
    ...
    std::string names = msg.params[3];
    if (!names.empty() && names[0] == ':')
      names.erase(0, 1);
    std::istringstream nss(names);
    std::string token;
    auto &ops = channel_ops[chan];
    while (nss >> token)
  ...
  if (msg.command == "PRIVMSG" && msg.params.size() >= 2) {
    ...
    if (!text.empty() && text[0] == '!') { // command
      ...
      if (it != commands.end()) {
        std::string reply_target =
            (target == currentNick && !sender_nick.empty()) ? sender_nick
                                                            : target;
        last_sender_nick = sender_nick;
        it->second(reply_target, args);
      }
    }
    if (!sender_nick.empty() && sender_nick != currentNick && target.size() &&
        target[0] == '#' && (text.empty() || text[0] != '!')) {
  ...
  if (msg.command == "INVITE" && msg.params.size() >= 2) {
    ...
    if (invitee == currentNick && channel.size()) {
      if (std::find(cfg.channels.begin(), cfg.channels.end(), channel) ==
          cfg.channels.end())
...
void Bot::run() {
  ...
  while (!g_stop) {
    ...
    if (sockfd < 0) {
      ++retries;
      std::cerr << "[bot] connect failed (" << retries << "/" << MAX_RETRIES
                << "), retry in " << backoff << "s\n";
    ...
    while (!g_stop) {
      ...
      if (pfd.revents & (POLLERR | POLLHUP | POLLNVAL)) {
        std::cerr
            << "[bot] socket closed (treating as server shutdown, exiting)\n";
        g_stop = 1;
        break;
      }
      if (pfd.revents & POLLIN) {
        ...
        if (n <= 0) {
          std::cerr
              << "[bot] recv <= 0 (treating as server shutdown, exiting)\n";
          ::close(sockfd);
          sockfd = -1;
          g_stop = 1;
          break;
